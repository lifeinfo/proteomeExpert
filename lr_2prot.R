...
mylm<-function(x,y){
  ...
  return(list(mylr=mylr,rSqured=mylr.summary$r.squared,fstatistic=mylr.summary$fstatistic[1],p=mylr.summary$coefficients[2,"Pr(>|t|)"]
  ))
}
  ...
  prot.matrix<-lapply(prot.group,function(d){
    ...
    if(is.null(d)){}
      ...
      for(i in 2:nrow(d)){
        ...
        if(lr.results$rSqured>0.36 & lr.results$p<0.05 & length(xy0.index)>0 & length(xy.index)>2){
          
          tryCatch(y.predict<-predict(lr.results$mylr,new=data.frame(x=unlist(x[xy0.index]))),
                   error = function (e) {print(d[i,]);stop()}
          )
