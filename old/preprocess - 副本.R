...
auto_preprocess<-function(filename="peptides.txt",tech_rep_f="technical_rep.txt",psep="\t",tsep="\t",pheader=TRUE,theader=FALSE,
                          bheader=TRUE,bsep="\t"){
  ...
  pep.data.log2[,2:ncol(pep.data.log2)]<-log2(as.matrix(pep.data.log2[,2:ncol(pep.data.log2)]))
  ...
  pep_order2.top3<-pep_order2.top3[grep("^1/",pep_order2.top3$prot),]
  pep_order2.top3<-pep_order2.top3[c("prot","tg",colnames(pep_order2.top3)[3:ncol(pep_order2.top3)])]
