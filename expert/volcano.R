  ...
  function(data,
    ...
    df8$fd <-
      apply(df8, 1, function(x)
        log2(mean(x[H_set], na.rm = T) / mean(x[L_set], na.rm = T)))
    x <- c(0.0, 0.0)
    df9 <- data
    df9[is.na(df9)] <- 0
    
    x <-
      apply(df9, 1, function(v) {
        t.test(v[H_set], v[L_set], paired = F, var.equal = F)$p.value
      })
    if (adjust == FALSE) {
      df8$P_value <- x
    } else{
